...
export default function CalendarScreen({ navigation }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      let stopsResponse
      try {
        stopsResponse = await fetch('http://10.0.2.2:3000/vasttrafik/getNearbyStops', {
          body: JSON.stringify({ lat: locationCoords.coords.latitude, lng: locationCoords.coords.longitude }),
          headers: {
      ...
      let depResponse
      try {
        depResponse = await fetch('http://10.0.2.2:3000/vasttrafik/getDepartures', {
  ...
  return (
    <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#333' }}>
      <View>
        ...
        <Text style={{ color: 'white', textAlign: 'center', fontSize: 24, marginBottom: 10 }}>Avgångstavla</Text>
        ...
        {currentStopLines && (
          ...
          <Picker
            ...
            style={{ height: 50, width: 375, color: 'white', backgroundColor: '#666' }}
          ...
          <ScrollView style={{ maxHeight: '80%', alignContent: 'center' }}>
            {currentStopLines.map((line, i) => {
              ...
              return (
                <View
                  key={i}
                  style={{ backgroundColor: i % 2 === 0 ? '#555' : '#444', paddingVertical: 6 }}>
                  <View style={{ flexDirection: 'row' }}>
                    <Text style={{ backgroundColor: line.fgColor, color: line.bgColor, paddingHorizontal: 8, paddingVertical: 2 }}>{line.sname}</Text>
                    <Text style={{ color: 'white', marginLeft: 7 }}>mot {line.direction}</Text>
                  </View>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'flex-start', marginVertical: 4 }}>
                    {line.departures.map((dep, i) => {
                      ...
                      return (<Text style={{ color: '#ccc', marginLeft: i > 0 ? 7 : 0 }} key={i}>{dep.time}</Text>)
                    })}
                  </View>
